...
public class TextRendererImproved_bak {
    ...
    private static final String BG_VERTEX_SHADER = """
        ...
        void main() {
            vec4 screenPos = projection * view * vec4(aWorldPos, 0.0, 1.0);
            vec2 offset = (aPos * aSize) / windowSize * 2.0;
            
            gl_Position = vec4(screenPos.xy + offset, 0.0, 1.0);
            Color = aColor;
        }
        """;
    ...
    public TextRendererImproved_bak() {
    ...
    private void renderBackgrounds(Camera2D camera, Window window) {
        ...
        for (Text text : texts) {
            ...
            float padding = 4.0f * text.scale;
            
            // Position et taille du rectangle de fond
            float rectX = text.worldX - padding;
            float rectY = text.worldY - padding;
            float rectWidth = width + padding * 2;
            float rectHeight = height + padding * 2;
            
            // Quad pour le fond (4 vertices)
            // Format: x, y, r, g, b, a, worldX, worldY, width, height
            
            // Bottom-left
            bgVertexBuffer.put(0.0f).put(0.0f)
                         .put(text.bgR).put(text.bgG).put(text.bgB).put(text.bgA)
                         .put(rectX).put(rectY)
                         .put(rectWidth).put(rectHeight);
            
            // Bottom-right
            bgVertexBuffer.put(1.0f).put(0.0f)
                         .put(text.bgR).put(text.bgG).put(text.bgB).put(text.bgA)
                         .put(rectX).put(rectY)
                         .put(rectWidth).put(rectHeight);
            
            // Top-right
            bgVertexBuffer.put(1.0f).put(1.0f)
                         .put(text.bgR).put(text.bgG).put(text.bgB).put(text.bgA)
                         .put(rectX).put(rectY)
                         .put(rectWidth).put(rectHeight);
            
            // Top-left
            bgVertexBuffer.put(0.0f).put(1.0f)
                         .put(text.bgR).put(text.bgG).put(text.bgB).put(text.bgA)
                         .put(rectX).put(rectY)
                         .put(rectWidth).put(rectHeight);
        }
        ...
        // Upload indices (ou utiliser un EBO pré-créé)
        IntBuffer indexBuffer = BufferUtils.createIntBuffer(indices.length);
        indexBuffer.put(indices).flip();
        
        glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, ebo);  // Réutiliser l'EBO du texte ou en créer un séparé
